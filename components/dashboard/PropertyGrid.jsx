...
export default function PropertyGrid({ properties, loading, isLoadingMore, onSelect }) {
  ...
  return (
    <>
      ...
      {/* Per-page skeleton row appended at the bottom during infinite scroll */}
      {isLoadingMore && (
        <div
        ...
        >
          {skeletonItems.slice(0, 3).map((_, i) => (
            <div key={`append-skeleton-${i}`} className="bg-white rounded-3xl border border-navy-200 overflow-hidden h-[420px] animate-pulse">
              <div className="h-48 bg-navy-100" />
              <div className="p-4 space-y-3">
                <div className="h-5 bg-navy-100 rounded-lg w-3/4" />
                <div className="h-4 bg-navy-100 rounded-lg w-1/2" />
