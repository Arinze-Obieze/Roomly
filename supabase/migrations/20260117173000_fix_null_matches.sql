...
BEGIN
    ...
    -- If no preferences found, return NULL score to indicate "Profile Incomplete"
    IF seeker_prefs IS NULL THEN
        RETURN QUERY SELECT id, NULL::numeric, '{}'::jsonb FROM properties WHERE is_active = true;
        RETURN;
    END IF;
    ...
    WITH scored_properties AS (
        SELECT 
            p.id,
            -- ... (rest of the logic remains the same, I will just copy the previous logic but ensure I don't break it)
            -- To keep this update clean, I'll just paste the main CTE logic again
            ...
            -- Move-in Check
            CASE 
                WHEN seeker_prefs.move_in_window = 'immediately' AND p.available_from > (CURRENT_DATE + INTERVAL '14 days') THEN 0.5
            ...
            (
                ...
                (CASE 
                    ...
                    WHEN h_life.smoking_status = 'no' THEN 100
                    ELSE 0
                END) * 0.25 + 
        ...
        WHERE p.is_active = true
          AND p.listed_by_user_id != seeker_id
    )
    SELECT 
        id,
        (
