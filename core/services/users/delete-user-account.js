...
export async function deleteUserAccount(admin, userId, authUser = null) {
  ...
  const avatarPath =
    ...
    unwrapPath(authUser?.user_metadata?.avatar_url) ||
    unwrapPath(authUser?.user_metadata?.picture);
  ...
  await mustSucceed(
    ...
    'Failed to delete reports created by user'
  );
  ...
  await mustSucceed(
    admin.from('people_interests').delete().or(`initiator_user_id.eq.${userId},target_user_id.eq.${userId}`),
    'Failed to delete people interests'
  );
  await mustSucceed(
  ...
  await mustSucceed(
    admin.from('matching_user_features').delete().eq('user_id', userId),
    'Failed to delete user feature snapshot'
  );
  await mustSucceed(
    admin.from('fcm_tokens').delete().eq('user_id', userId),
    'Failed to delete FCM tokens'
  );

  if (propertyIds.length > 0) {
    await mustSucceed(
      admin.from('people_interests').delete().in('context_property_id', propertyIds),
      'Failed to delete people interests on user properties'
    );
    await mustSucceed(
    ...
    await mustSucceed(
      admin.from('matching_property_features').delete().in('property_id', propertyIds),
      'Failed to delete property feature snapshots'
    );
    await mustSucceed(
