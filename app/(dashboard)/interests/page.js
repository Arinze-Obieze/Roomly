...
export default function InterestsPage() {
  ...
  return (
    <main className="container mx-auto px-4 py-8 max-w-5xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-900 mb-2">Room Interests</h1>
        <p className="text-slate-500">Manage interactions and unlock mutual private details.</p>
      </div>
      ...
      <div className="flex bg-slate-100 p-1.5 rounded-2xl mb-8 w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold transition-all ${
            activeTab === 'seeker' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
          }`}
        >
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-3xl border border-slate-100">
           <div className="bg-slate-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4">
              <MdAccessTime className="text-slate-300 text-4xl" />
           </div>
           <h3 className="text-xl font-bold text-slate-900">No interests found</h3>
             ...
             <div key={item.id} className="bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
               {activeTab === 'seeker' ? (
                 ...
                 <div onClick={() => router.push(`/rooms/${item.property.id}`)} className="cursor-pointer">
                    <div className="relative h-40">
                        {item.property.property_media?.[0]?.url ? (
                            ...
                            <div className="w-full h-full bg-slate-100 flex items-center justify-center">
                                <MdFavorite className="text-slate-200 text-4xl" />
                            </div>
                        )}
                    ...
                    <div className="p-5">
                        <h4 className="font-bold text-slate-900 line-clamp-1 mb-1">{item.property.title}</h4>
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <div className={`w-8 h-8 rounded-full bg-slate-100 overflow-hidden ${item.shouldMask ? 'blur-[2px]' : ''}`}>
                                    {item.property.users?.profile_picture && (
                                        <img src={item.property.users.profile_picture} className="w-full h-full object-cover" />
                                    )}
                                </div>
                                <span className="text-xs font-bold text-slate-700">{item.property.users?.full_name}</span>
                            </div>
                 ...
                 <div className="p-5">
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-3">
                            <div className={`w-12 h-12 rounded-full bg-cyan-50 flex items-center justify-center overflow-hidden border-2 border-white shadow-sm ${item.shouldMask ? 'blur-[3px]' : ''}`}>
                                {item.seeker.profile_picture ? (
                                    <img src={item.seeker.profile_picture} className="w-full h-full object-cover" />
                                ) : (
                                    <MdPerson className="text-cyan-600 text-2xl" />
                                )}
                            </div>
                            <div>
                    ...
                    <div className="bg-slate-50 rounded-2xl p-3 mb-4">
                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">Interested in</div>
                        <div className="text-sm font-bold text-slate-700 line-clamp-1">{item.property.title}</div>
                    </div>
                    ...
                    {item.status === 'pending' ? (
                        <div className="flex gap-2">
                            <button 
                                ...
                                className="flex-1 bg-cyan-600 text-white font-bold py-2 rounded-xl text-xs hover:bg-cyan-700 transition-colors shadow-lg shadow-cyan-600/10 active:scale-95"
                            >
                                Accept
                            </button>
                            <button 
                                onClick={() => handleUpdateStatus(item.id, 'rejected')}
                                className="flex-1 bg-slate-100 text-slate-600 font-bold py-2 rounded-xl text-xs hover:bg-slate-200 transition-colors active:scale-95"
                            >
                                Decline
                            </button>
                        </div>
                        ...
                        <button 
                            ...
                            className="w-full bg-slate-900 text-white font-bold py-2 rounded-xl text-xs hover:bg-slate-800 transition-colors active:scale-95"
                        >
                            Open Conversation
                        </button>
                    )}
