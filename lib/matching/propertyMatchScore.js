...
function scoreHostHarmony(property, seekerLifestyle, seekerPrefs, hostLifestyle, hostMeta, weight) {
  ...
  let cleanPoints = maxPoints * 0.25; // 25% of harmony points
  if (cleanReqScore > 0) {
      if (hostCleanliness >= cleanReqScore) earnedPoints += cleanPoints; // Complete pass
      else earnedPoints += (cleanPoints * 0.3); // Host is messier than preferred (Penalty)
  } else {
  ...
  let guestPoints = maxPoints * 0.25; // 25% of harmony points
  ...
  let demoPoints = maxPoints * 0.25; // 25% of harmony points
  let demoMatches = [];
  ...
  let frictionPoints = maxPoints * 0.25; 
  let fPoints = 0;
  
  // Smoking: Does the property allow it, and does the seeker accept it?
  const propDealBreak = normalizeDealBreakers(property.deal_breakers);
  const seekerAcceptsSmoking = (seekerPrefs?.accepted_smoking || []).length > 0;
  
  if (seekerLifestyle?.smoking_status === 'no') {
      fPoints += frictionPoints * 0.5; // Non-smoker is always low friction
  } else if (!propDealBreak.includes('smoking')) {
      fPoints += frictionPoints * 0.3; // Seeker smokes, property allows (minor friction)
  }

  // Pets: Does seeker accept pets assuming the property has them?
  const propAcceptsPets = !propDealBreak.includes('no_pets');
  const seekerAcceptsPets = !!seekerPrefs?.accepted_pets;
  if (seekerAcceptsPets || !propAcceptsPets) {
      fPoints += frictionPoints * 0.5; // Harmonic! Either seeker loves pets, or property has none.
  } else if (!seekerAcceptsPets && propAcceptsPets) {
      // Small penalty: Property allows pets, seeker doesn't like them. Friction.
      fPoints += frictionPoints * 0.2; 
  }
