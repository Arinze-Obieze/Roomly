...
export function propertyMatchConfidence(
  ...
  if (hasMeaningfulSignals(seekerLifestyle)) availableSignals += 3;
  if (hasMeaningfulSignals(seekerPrefs)) availableSignals += 3;
  if (seekerMeta?.gender || seekerMeta?.date_of_birth || seekerMeta?.occupation) availableSignals += 1;

  if (isSharedSpace) {
    if (hasMeaningfulSignals(hostLifestyle)) availableSignals += 2;
...
function hasMeaningfulSignals(value) {
  ...
  return Object.entries(value).some(([key, entry]) => {
    ...
    if (typeof entry === 'object') return hasMeaningfulSignals(entry);
    return false;
  });
}
